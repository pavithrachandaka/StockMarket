...
class UniversalDataCollector:
    """
    Collects and preprocesses ANY market data from Yahoo Finance.
    """
    
    def __init__(self, symbol: str = "^GSPC", period: str = "5y"):
        """
...
def collect_sample_data() -> pd.DataFrame:
    """
    Collect sample market data for demonstration.
...
def get_market_summary(symbol: str = "^GSPC") -> dict:
    """
    Get a summary of current market conditions.
    
    Args:
        symbol: Market symbol to get summary for
...
if __name__ == "__main__":
    # Example usage
    print("Universal Market Data Collector")
    print("=" * 50)
    
    # Example markets
    markets = ['^GSPC', 'BTC-USD', 'AAPL', 'GC=F', '^FTSE']
    
    for symbol in markets:
        print(f"\n📊 Testing {symbol}:")
        collector = UniversalDataCollector(symbol=symbol)
        data = collector.fetch_data()
